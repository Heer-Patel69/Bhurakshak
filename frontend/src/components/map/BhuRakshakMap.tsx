...
// Shared dark-panel wrapper for popup HTML so every popup matches the app's visual language
function popupShell(innerHtml: string) {
  return `<div style="background:#0f172a;border:1px solid #1e293b;border-radius:12px;padding:10px 12px;min-width:180px;font-family:inherit;">${innerHtml}</div>`;
}
...
export function BhuRakshakMap({
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      mapRef.current = map;
      setMapLoaded(true);
      initializeDataSources(map).then(() => setDataReady(true));
    });
            ...
            .setHTML(
              popupShell(`
                <div style="font-size:11px;line-height:1.5;">
                  <div style="font-weight:700;color:#fb7185;margin-bottom:2px;">Landslide Event ${props.event_id || ''}</div>
                  <div style="color:#cbd5e1;"><strong style="color:#94a3b8;">Location:</strong> ${props.location_name || 'Aizawl Region'}</div>
                  <div style="color:#cbd5e1;"><strong style="color:#94a3b8;">Recorded:</strong> ${props.date ? props.date : 'Date unavailable'}</div>
                  <div style="color:#cbd5e1;"><strong style="color:#94a3b8;">Type:</strong> ${props.landslide_type || 'Unknown'}</div>
                  <div style="color:#64748b;font-size:10px;margin-top:4px;">Source: ${props.source || 'GSI Historical Inventory'}</div>
                </div>
              `)
            )
            .addTo(map);
        ...
        map.on('click', 'settlements-layer', (e) => {
          ...
          new maplibregl.Popup()
            ...
            .setHTML(
              popupShell(`
                <div style="font-size:11px;line-height:1.5;">
                  <div style="font-weight:700;color:#38bdf8;margin-bottom:2px;">${props.name}</div>
                  <div style="color:#cbd5e1;"><strong style="color:#94a3b8;">Type:</strong> Settlement / Village</div>
                  <div style="color:#cbd5e1;"><strong style="color:#94a3b8;">Status:</strong> ${props.status || 'Connected'}</div>
                  <div style="color:#64748b;font-size:10px;margin-top:4px;">Source: Geofabrik OSM Aizawl</div>
                </div>
              `)
            )
            .addTo(map);
        ...
        map.on('click', 'facilities-layer', (e) => {
          ...
          new maplibregl.Popup()
            ...
            .setHTML(
              popupShell(`
                <div style="font-size:11px;line-height:1.5;">
                  <div style="font-weight:700;color:#34d399;margin-bottom:2px;">🏥 ${props.name}</div>
                  <div style="color:#cbd5e1;"><strong style="color:#94a3b8;">Type:</strong> ${props.facility_type || 'Emergency Facility'}</div>
                  <div style="color:#64748b;font-size:10px;margin-top:4px;">Source: OSM Aizawl Critical Infrastructure</div>
                </div>
              `)
            )
            .addTo(map);
          ...
          map.on('click', 'roads-base-layer', (e) => {
            ...
            new maplibregl.Popup()
              ...
              .setHTML(
                popupShell(`
                  <div style="font-size:11px;line-height:1.5;">
                    <div style="font-weight:700;color:#e2e8f0;margin-bottom:2px;">🛣️ ${props.name || 'Unnamed Road Segment'}</div>
                    <div style="color:#cbd5e1;"><strong style="color:#94a3b8;">Status:</strong> ${props.closure_confirmed ? '🔴 Confirmed Closed' : (props.status || 'Open')}</div>
                    <div style="color:#cbd5e1;"><strong style="color:#94a3b8;">Highway Type:</strong> ${props.highway || 'local'}</div>
                    <div style="color:#64748b;font-size:10px;margin-top:4px;">Source: Geofabrik OSM Aizawl</div>
                  </div>
                `)
              )
  ...
  // Update selected coordinates pin marker — custom SVG drop pin with pulse ring
  useEffect(() => {
    ...
    if (selectedCoordinates) {
      if (!selectedMarkerRef.current) {
        ...
        el.innerHTML = `
          <span style="position:absolute;inset:0;margin:auto;width:28px;height:28px;border-radius:9999px;background:rgba(244,63,94,0.35);animation:brk-pulse 1.6s ease-out infinite;"></span>
          <svg width="28" height="28" viewBox="0 0 28 28" style="position:relative;filter:drop-shadow(0 2px 4px rgba(0,0,0,0.5));">
            <circle cx="14" cy="14" r="7" fill="#f43f5e" stroke="#ffffff" stroke-width="2.5" />
          </svg>
          <style>
            @keyframes brk-pulse {
  ...
  return (
    <div className="relative w-full h-full min-h-[400px] overflow-hidden bg-slate-950">
      ...
      {/* Loading overlay — shown until map + first data layers are in */}
      {(!mapLoaded || !dataReady) && (
        <div className="absolute inset-0 z-40 flex flex-col items-center justify-center gap-3 bg-slate-950/90 backdrop-blur-sm">
          <div className="w-10 h-10 border-2 border-sky-400 border-t-transparent rounded-full animate-spin" />
          <span className="text-xs font-medium text-slate-400 tracking-wide">
            {!mapLoaded ? 'Loading map…' : 'Loading risk & terrain layers…'}
          </span>
        </div>
      )}
      ...
      {/* Floating Action Controls — grouped as one panel */}
      <div className="absolute top-4 left-16 z-20 flex flex-col gap-0.5 bg-slate-900/90 border border-slate-700 rounded-xl shadow-xl backdrop-blur-md overflow-hidden">
        <button
          onClick={handleLocateUser}
          disabled={isLocating}
          className="p-2.5 text-sky-400 hover:bg-slate-800 transition-colors"
